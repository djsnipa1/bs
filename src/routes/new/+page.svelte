...
<script>
  ...
  import { injectStyleString } from '$lib/util/zIndexes.js';
  ...
  injectStyleString(browser, Z_INDEX_LAYERS);
...
<div class="pointer-events-none relative h-screen w-full" id="app">
  ...
  <div class="absolute inset-0 h-screen w-full" id="debugButtons">
    <!-- comment back on -->
    {#if $debugModeEnabled}
      <DebugButtons
  ...
  <div class="absolute inset-0 h-screen w-full" id="inputBoxRework">
    <InputBoxRework bind:this={fillFunction} />
  </div>
  <!-- old div here </div> -->
  ...
  >
    ...
    <nav
      class="absolute top-0 flex h-12 w-full items-center justify-between rounded-sm bg-[linear-gradient(180deg,#ffe636_0%,#ffd430_5%,#ffc12b_10%,#ffb72c_25%,#ffa51a_40%,#f6a200_60%,#f59c00_75%,#f39500_90%,#f28d00_95%,#f78d00_100%)] opacity-100 shadow-lg"
      id="urlButton"
    >
    ...
    {#if $isPlayerControlsReady}
      <div class="translate-y-1/2" transition:fade id="playerControlsTest">
        <PlayerControlsTest />
      </div>
    {/if}
    <!-- end comment back on -->
  ...
  <!-- comment back on -->
  <div
    class="absolute right-0 top-0 h-full w-screen border-2 border-green-500"
    id="circleImage"
  >
  ...
  <div
    class="absolute right-0 top-0 h-full w-screen border-2 border-cyan-500"
    id="helpScreen"
  >
